...
// TestMatterDetail_GetMatter_ExposesSourceMsgIDs is the service-to-wire test
// that issue #43 round-3 review missed and that subsequent manual API testing
// flagged: GET /api/v1/matters/:id must emit source_msg_ids in the response.
// The MatterDetail DTO embeds *model.Matter, so any drift in the Matter JSON
// tag would silently hide the field from clients.
func TestMatterDetail_GetMatter_ExposesSourceMsgIDs(t *testing.T) {
	matter := &model.Matter{
		ID:           "t1",
		SpaceID:      "sp1",
		CreatorID:    "owner",
	...
	if !strings.Contains(string(b), `"source_msg_ids":["msg-a","msg-b"]`) {
		t.Fatalf("MatterDetail JSON missing populated source_msg_ids; got %s", b)
	}
}
...
func TestMatterDetail_GetMatter_LegacyMatterStillExposesEmptyArray(t *testing.T) {
	...
	if !strings.Contains(string(b), `"source_msg_ids":[]`) {
		t.Fatalf("legacy matter JSON must render source_msg_ids as []; got %s", b)
	}
}
