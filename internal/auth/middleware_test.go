...
// keys are set so SpaceMiddleware can enforce on
...
func TestAuthMiddleware_User_HappyPath(t *testing.T) {
	...
	if !resp["ctx_incl"].(bool) {
		t.Fatalf("SDK CtxKeyContextIncluded not set; SpaceMiddleware would silently no-op")
	}
...
// keys plumbed for SpaceMiddleware + the verified-spaces list
