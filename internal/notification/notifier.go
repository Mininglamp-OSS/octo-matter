...
// callers fire-and-forget via SafeGo.
type Notifier interface {
	...
	NotifyStatusChanged(todo *model.Todo, actorID, actorName string, assigneeIDs []string)
	...
	NotifyCommentAdded(todo *model.Todo, actorID, actorName string, assigneeIDs []string)
}
...
func (Noop) NotifyStatusChanged(_ *model.Todo, _, _ string, _ []string) {}
func (Noop) NotifyAssigneeAdded(_ *model.Todo, _, _ string)             {}
func (Noop) NotifyCommentAdded(_ *model.Todo, _, _ string, _ []string)  {}

// SafeGo runs fn in a goroutine with panic recovery. Use this for all
// notification calls so a buggy notifier cannot crash the process.
func SafeGo(fn func()) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
